	...
	Drupal.behaviors.setupFeedbackForm = {
		attach: function(context, settings) {
			...
			if ($('#webform-client-form-13321').length) {
				...
				// @TODO: Find a more accurate way of determining if the 
				// form has been submitted - alerts could relate to something else
				if ($('#messages-console .alert-success').length) {
					// If a 'Success' alert is found, remove the form, then stop right there. 
					$form.remove();
					return
				} else {
